...
def build_location_master(df: pd.DataFrame) -> pd.DataFrame:
    ...
    OSM road class enrichment:
    If data/road_class_lookup.json exists (generated by scripts/fetch_road_classes.py),
    we merge highway, tier, road_weight_osm, is_oneway, osm_road_name into the master.
    These fields are returned by the /locations endpoint and displayed in the frontend.
    """
    ...
    # ── OSM Road Class Enrichment ─────────────────────────────────
    # LOGIC: If the road_class_lookup.json was generated by fetch_road_classes.py,
    # we merge it into the location master here. This adds:
    #   highway        — OSM road type string ("primary", "secondary", etc.)
    #   road_tier      — integer 0-5 (0=motorway, 5=service lane)
    #   road_weight_osm — 0.15-1.0 (used as fallback/override for 1/lane_count)
    #   is_oneway      — boolean flag
    #   osm_road_name  — the OSM road name (e.g. "Outer Ring Road")
    # If the file doesn't exist, we skip silently — the API still works.
    if ROAD_CLASS_LOOKUP.exists():
        try:
            with open(ROAD_CLASS_LOOKUP, encoding="utf-8") as f:
                lookup = json.load(f)
            road_df = pd.DataFrame.from_dict(lookup, orient="index").reset_index()
            road_df = road_df.rename(columns={
                "index":       "location_key",
                "highway":     "highway",
                "tier":        "road_tier",
                "road_weight": "road_weight_osm",
                "road_label":  "road_label",
                "is_oneway":   "is_oneway",
                "osm_name":    "osm_road_name",
                "snap_dist_m": "osm_snap_dist_m",
            })
            keep_cols = ["location_key", "highway", "road_tier", "road_weight_osm",
                         "road_label", "is_oneway", "osm_road_name"]
            road_df = road_df[[c for c in keep_cols if c in road_df.columns]]
            master = master.merge(road_df, on="location_key", how="left")
            matched = master["highway"].notna().sum()
            log.info(
                "OSM road class merged: %d / %d locations matched (%.1f%%)",
                matched, len(master), 100 * matched / max(len(master), 1)
            )
        except Exception as e:
            log.warning("Failed to load road_class_lookup.json: %s — skipping OSM enrichment", e)
    else:
        log.info(
            "road_class_lookup.json not found at %s. "
            "Run scripts/fetch_road_classes.py to enable OSM road enrichment.",
            ROAD_CLASS_LOOKUP
        )
